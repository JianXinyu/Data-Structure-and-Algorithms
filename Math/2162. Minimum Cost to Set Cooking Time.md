...
# summary
“用数组 $\textit{digits}$ 按顺序表示四位输入的每一位” 值得学习
# Analysis
> Lemma: 对于某一个目标秒数，**至多只有两种**按键方案有可能是最优的。
...
同时，我们用 $\textit{mm}, \textit{ss} (0 \le \textit{ss} \le 59)$ 来唯一表示目标时间对应的分和秒数。那么，我们有：
...
那么，最优的方案只能是上述两种（如果存在）中花费较小的方案。我们只需要计算上述两种方案对应的最小花费即可。
...
对于两种方案，我们首先判断取值范围，如果合法，则计算对应的最小花费。最终，我们返回所有可行的花费中较小的作为答案。为了方便判断 $m, s$ 的合法性，我们可以在函数 $\textit{cost}(m, s)$ 中对于不合法的 $m, s$ 返回一个极大的数即可。
